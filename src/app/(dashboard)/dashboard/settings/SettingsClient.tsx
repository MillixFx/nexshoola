...
type School = { id: string; name: string; slug: string; address: string | null; phone: string | null; email: string | null; headmaster: string | null; country: string; currency: string; timezone: string; startYear: number | null; plan: string } | null
...
export default function SettingsClient({ school }: { school: School }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/settings", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: school?.id, ...form }) })
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                <input className="flex-1 px-4 py-2.5 text-sm outline-none" required value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value }))} placeholder="gis" />
