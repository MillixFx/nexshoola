...
export default function PayrollClient({ payslips: initial, teachers, schoolId, schoolName, isTeacher = false }: {
  payslips: Payslip[]; teachers: Teacher[]; schoolId: string; schoolName: string; isTeacher?: boolean
}) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={isTeacher ? "My Payslips" : "Payroll"}
        description={`${payslips.length} payslip${payslips.length !== 1 ? "s" : ""}${!isTeacher ? ` · ${teachers.length} staff` : ""}`}
        action={
          !isTeacher ? (
      ...
      {payslips.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-14 flex flex-col items-center text-center shadow-sm">
          ...
          <p className="text-sm text-gray-500 mb-3">{isTeacher ? "Your payslips will appear here once generated by admin" : "Generate monthly payslips for your teaching staff"}</p>
          {!isTeacher && <button onClick={openAdd} className="text-sm font-semibold text-indigo-600 hover:underline">+ Generate Payslip</button>}
        </div>
      ) : (
                    ...
                    <tr key={p.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 justify-end">
                          ...
                          {!isTeacher && p.status === "DRAFT" && <button onClick={() => updateStatus(p.id, "ISSUED")} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg" title="Mark Issued"><Check className="w-3.5 h-3.5" /></button>}
                          {!isTeacher && p.status === "ISSUED" && <button onClick={() => updateStatus(p.id, "PAID")} className="p-1.5 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg" title="Mark Paid"><Banknote className="w-3.5 h-3.5" /></button>}
                          {!isTeacher && <button onClick={() => openEdit(p)} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg"><Pencil className="w-3.5 h-3.5" /></button>}
                          {!isTeacher && <button onClick={() => handleDelete(p.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg"><Trash2 className="w-3.5 h-3.5" /></button>}
                        </div>
                      </td>
                    </tr>
                  )
