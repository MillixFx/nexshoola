...
import { DollarSign, Plus, TrendingUp, TrendingDown, CreditCard, FileText, Loader2, X, Printer, ExternalLink } from "lucide-react"
...
interface Props {
  transactions: Transaction[]; feeItems: FeeItem[]; students: Student[]; classes: Class[]
  schoolId: string; isAdmin?: boolean; userEmail?: string
}

const TABS = ["Overview", "Transactions", "Fee Items"] as const

export default function FinanceClient({ transactions: initialTx, feeItems: initialFees, students, classes, schoolId, isAdmin = true, userEmail = "" }: Props) {
    ...
    try {
      const res = await fetch("/api/paystack/initialize", {
        ...
        body: JSON.stringify({
          ...
          type: "FEE",
          metadata: { feeItemId: paystackForm.feeItemId, schoolId, email: userEmail },
        }),
      })
      if (!res.ok) throw new Error((await res.json()).error)
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            ...
            {isAdmin && (
              <>
                <button onClick={handlePrint} className="flex items-center gap-2 border border-gray-200 text-gray-700 text-sm font-semibold px-4 py-2.5 rounded-xl hover:bg-gray-50">
              ...
              {transactions.slice(0, 10).map(t => (
                <div key={t.id} className="flex items-center gap-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900">{t.feeItemTitle ?? t.description ?? t.note ?? "Payment"}</p>
                    <p className="text-xs text-gray-400">{t.studentName ?? "—"} · {formatDate(t.date)} · {t.method}</p>
                  </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {transactions.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-gray-400">No transactions.</td></tr>
              ) : transactions.map(t => (
                <tr key={t.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{t.feeItemTitle ?? t.description ?? t.note ?? "—"}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={handlePayment} className="p-6 space-y-4">
              ...
              <div><label className="label">Amount *</label><input className="input" type="number" min="0" step="0.01" required value={payForm.amount} onChange={e => setPayForm(f => ({ ...f, amount: e.target.value }))} placeholder="0.00" /></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={handleFeeItem} className="p-6 space-y-4">
              ...
              <div><label className="label">Amount *</label><input className="input" type="number" min="0" required value={feeForm.amount} onChange={e => setFeeForm(f => ({ ...f, amount: e.target.value }))} placeholder="500.00" /></div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Powered by Paystack — secure card & mobile money</p>
              </div>
            ...
            <form onSubmit={handlePayOnline} className="p-6 space-y-4">
              ...
              <div><label className="label">Amount *</label>
                <div className="relative">
                  <input className="input pr-20" type="number" min="0" step="0.01" required
                    value={paystackForm.amount} onChange={e => setPaystackForm(f => ({ ...f, amount: e.target.value }))} placeholder="0.00" />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-medium">GHS</span>
                </div>
              </div>
