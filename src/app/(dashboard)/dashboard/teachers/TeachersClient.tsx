...
import { Plus, Pencil, Trash2, GraduationCap, CreditCard, Camera } from "lucide-react"
import PageHeader from "@/components/dashboard/PageHeader"
import DataTable, { Column } from "@/components/dashboard/DataTable"
import { cn } from "@/lib/utils"
import ConfirmModal from "@/components/dashboard/ConfirmModal"
import IDCard, { StaffCardData, SchoolInfo } from "@/components/dashboard/IDCard"
...
export default function TeachersClient({ teachers: initial, schoolId, school }: Props) {
  ...
  function openAdd() { setEditing(null); setForm(emptyForm); setPhotoPreview(""); setError(""); setOpen(true) }
  function openEdit(t: Teacher) {
    setEditing(t)
    setForm({
      name: t.user.name, email: t.user.email, phone: t.user.phone ?? "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!editing && !photoPreview) {
      setError("A passport photo is required for all staff members.")
      return
    }
    ...
    try {
      if (editing) {
        const res = await fetch(`/api/teachers/${editing.id}`, {
          ...
          body: JSON.stringify({ ...form, isActive: true, photo: photoPreview || undefined }),
        })
        ...
        const res = await fetch("/api/teachers", {
          ...
          body: JSON.stringify({ ...form, schoolId, photo: photoPreview || undefined }),
        })
        if (!res.ok) throw new Error((await res.json()).error)
  ...
  const columns: Column<Teacher>[] = [
    {
      key: "user", label: "Teacher", primary: true, render: t => (
        <div className="flex items-center gap-3">
          {t.photo
            ? <img src={t.photo} alt={t.user.name} className="w-8 h-8 rounded-full object-cover shrink-0" />
            : (
    ...
    { key: "phone",       label: "Phone",      render: t => t.user.phone ?? "—" },
    {
      key: "isActive", label: "Status", render: t => (
        <span className={cn("inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full",
          t.user.isActive ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600")}>
    ...
    <div className="space-y-6">
      ...
      {teachers.length === 0 ? (
        ...
        <DataTable
          ...
          viewKey="teachers"
          photoUrl={t => t.photo}
          initials={t => t.user.name.split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 2)}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex flex-col items-center gap-3 pb-4 border-b border-gray-100">
                <div className="relative">
                  {photoPreview ? (
                    <img src={photoPreview} alt="Passport photo"
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-400 mt-1">Required for all staff members</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  ...
                  <input className="input" required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Mr. Kofi Mensah" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input className="input" value={form.designation} onChange={e => setForm(f => ({ ...f, designation: e.target.value }))} placeholder="e.g. Senior Maths Teacher, Librarian…" />
                </div>
                <div>
                  <label className="label">Department</label>
                  <input className="input" value={form.department} onChange={e => setForm(f => ({ ...f, department: e.target.value }))} placeholder="Science" />
                </div>
                <div className="sm:col-span-2">
                  <label className="label">Qualification</label>
                  <input className="input" value={form.qualification} onChange={e => setForm(f => ({ ...f, qualification: e.target.value }))} placeholder="B.Ed. Mathematics" />
                </div>
                <div>
      ...
      {idCardTeacher && school && (
        <IDCard
          data={{
            type: "staff",
            name: idCardTeacher.user.name,
            photo: idCardTeacher.photo,
            role: idCardTeacher.user.role,
            staffId: idCardTeacher.teacherId ?? idCardTeacher.employeeId,
            designation: idCardTeacher.designation,
            department: idCardTeacher.department,
            qualification: idCardTeacher.qualification,
            joiningDate: idCardTeacher.joiningDate,
            phone: idCardTeacher.user.phone,
            email: idCardTeacher.user.email,
          } satisfies StaffCardData}
          school={school}
