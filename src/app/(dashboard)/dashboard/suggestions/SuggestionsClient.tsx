...
type Suggestion = { id: string; title: string; content: string; isAnonymous: boolean; status: string; createdAt: string | Date; user: { name: string; role: string } | null }
const emptyForm = { title: "", content: "", isAnonymous: false }
...
export default function SuggestionsClient({ suggestions: initial, schoolId }: { suggestions: Suggestion[]; schoolId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/suggestions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, schoolId }) })
    const s = await res.json()
    setSuggestions(prev => [s, ...prev]); setOpen(false); setForm(emptyForm); setSaving(false)
  }

  const STATUS_COLORS: Record<string, string> = { PENDING: "bg-amber-50 text-amber-700", REVIEWED: "bg-blue-50 text-blue-700", IMPLEMENTED: "bg-emerald-50 text-emerald-700", REJECTED: "bg-red-50 text-red-600" }
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400 mt-2">
                    {s.isAnonymous ? "Anonymous" : s.user?.name ?? "Unknown"} · {formatDate(s.createdAt)}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div><label className="label">Title *</label><input className="input" required value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Improve school library hours" /></div>
              <div><label className="label">Details *</label><textarea className="input min-h-[120px] resize-none" required value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} /></div>
