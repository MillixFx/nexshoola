  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? "bg-white/90 dark:bg-gray-900/90 backdrop-blur-md shadow-sm border-b border-indigo-100 dark:border-gray-800"
          : "bg-transparent"
      )}
    >
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2.5 group">
          ...
          <span className={cn("font-bold text-xl tracking-tight transition-colors", scrolled ? "text-gray-900 dark:text-white" : "text-white")}>
        ...
        <ul className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                className={cn("text-sm font-medium transition-colors hover:text-indigo-400", scrolled ? "text-gray-600 dark:text-gray-300" : "text-white/80")}
              >
                {link.label}
              </a>
            </li>
        ...
        <div className="md:hidden flex items-center gap-2">
          <ThemeToggle />
          <button
            className="p-2 rounded-lg text-gray-600 dark:text-gray-300 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-gray-800 transition-colors"
          ...
          <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1">
            {navLinks.map((link) => (
              <a
                ...
                className="py-2.5 px-3 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-gray-800 transition-colors"
              >
                {link.label}
              </a>
            ))}
            <div className="pt-3 mt-2 border-t border-gray-100 dark:border-gray-800 flex flex-col gap-2">
              {isLoggedIn ? (
                ...
                <>
                  <Link
                    href="/login"
                    className="py-2.5 px-3 rounded-lg text-sm font-medium text-center text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800"
                  >
                    Sign in
                  </Link>
                  <Link
